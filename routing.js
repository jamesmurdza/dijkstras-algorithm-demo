...
// ---------------------------------------------------------------------
// Alternate output for "show edge weights" off: instead of one smoothly
// rounded path per destination (uniform stroke-width, weight read off a
// label), the caller wants stroke THICKNESS to carry each hop's weight
// instead - which a single <path> can't do (SVG has no per-position
// stroke-width). So this returns one lane segment PER HOP, each a plain
// two-point line between the same taper/center points buildDestinationPathD
// already computes for that hop, tagged with that hop's own weight so the
// renderer can size each one independently.
//
// Deliberately NOT rounded-fillet-joined the way the combined path is -
// once adjacent hops can render at different thicknesses, a visible seam
// at the node where they meet is expected (and even useful, marking the
// junction) rather than something to smooth away.
// ---------------------------------------------------------------------
function buildDestinationHopSegments(frame, usage, destNode) {
  var path = pathTo(destNode, frame.dist, frame.prev);
  if (!path || path.length < 2) return null;

  function taperPointBefore(node, dir, perp, offset) {
    return { x: node.x - dir.x * TAPER_LEN + perp.x * offset, y: node.y - dir.y * TAPER_LEN + perp.y * offset };
  }
  function taperPointAfter(node, dir, perp, offset) {
    return { x: node.x + dir.x * TAPER_LEN + perp.x * offset, y: node.y + dir.y * TAPER_LEN + perp.y * offset };
  }
  ...
  for (var i = 0; i < path.length - 1; i++) {
    ...
    // The FROM end is a's exact center only for the very first hop (S);
    // every other hop starts from a taper point, since its FROM node is
    // itself an intermediate stop the lane is leaving. Symmetric for the
    // TO end and destNode.
    var startPt = (i === 0) ? a : taperPointAfter(a, dir, perp, offset);
    var endPt = (i === path.length - 2) ? b : taperPointBefore(b, dir, perp, offset);

    hops.push({
      d: 'M ' + pt(startPt) + ' L ' + pt(endPt),
...
// Convenience: build { destNode: [{d, weight, from, to}, ...] } for every
// currently-discovered destination - the per-hop-segments analog of
// buildAllPaths above.
function buildAllHopSegments(frame) {
  var usage = collectEdgeUsage(frame);
  var out = {};
  Object.keys(NODES).forEach(function (node) {
    if (node === START_NODE) return;
    var hops = buildDestinationHopSegments(frame, usage, node);
    if (hops) out[node] = hops;
  });
  return out;
}
